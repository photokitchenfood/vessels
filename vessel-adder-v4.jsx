...
// Ensure the stored value is always a full data URL (data:image/jpeg;base64,...).
// The catalog uses the IMAGES value directly as an <img> src, so it needs the prefix.
function ensureDataUrl(photoStr) {
  if (!photoStr) return null;
  if (photoStr.startsWith("data:")) return photoStr;
  // Raw base64 without prefix — add the standard JPEG prefix
  return "data:image/jpeg;base64," + photoStr;
}

function mergeAndDownload(rawJs, queue, color) {
  var colorFile = COLOR_FILE_MAP[color] || color.toLowerCase();

  // 1. Parse existing vessels array from raw JS
  var existingVessels = parseVessels(rawJs);

  // 2. Build clean vessel objects (no _photo) for new entries
  var newVesselObjs = queue.map(function(v) {
  ...
  // 3. Merge vessels: existing + new
  var mergedVessels = existingVessels.concat(newVesselObjs);
  var mergedVesselsJson = JSON.stringify(mergedVessels, null, 2);

  // 4. Replace the vessels array in the raw JS.
  // Use the structural anchor ];\n\nIMAGES to precisely locate the end of the vessels array.
  var vesselsStart = rawJs.search(/vessels\s*=\s*\[/);
  var vesselsAnchor = rawJs.search(/\];\s*\n\s*\n\s*(?:(?:const|var|let)\s+)?IMAGES\s*=/);
  var updatedJs;
  if (vesselsStart !== -1 && vesselsAnchor !== -1) {
    var vesselsEnd = rawJs.indexOf("];", vesselsAnchor) + 2;
    updatedJs = rawJs.slice(0, vesselsStart) + "vessels = " + mergedVesselsJson + ";" + rawJs.slice(vesselsEnd);
  } else {
    // Fallback: non-greedy replace (for files without an IMAGES block yet)
    updatedJs = rawJs.replace(
      /vessels\s*=\s*\[[\s\S]*?\];/,
      "vessels = " + mergedVesselsJson + ";"
    );
  }

  // 5. Merge new images into IMAGES object.
  // Keep the full data URL — the catalog uses IMAGES values directly as <img src>,
  // so the "data:image/jpeg;base64,..." prefix must be present.
  // FIX: Find the IMAGES closing brace with a regex tolerant of `}` with or without
  //      a trailing semicolon and any surrounding whitespace.
  ...
  if (newImageEntries) {
    // Find the IMAGES block opening brace
    var imagesMatch = updatedJs.match(/(?:(?:const|var|let)\s+)?IMAGES\s*=\s*\{/);
    if (imagesMatch) {
      var imagesOpenIdx = updatedJs.indexOf("{", updatedJs.indexOf(imagesMatch[0]));

      // FIX B: Find the closing brace of the IMAGES block using a regex that handles:
      //   - `};`  (closing brace + semicolon)
      //   - `}`   (bare closing brace, no semicolon — used in yellow-orange.js etc.)
      //   - any surrounding whitespace / newlines
      // We search from the character AFTER the opening brace so we don't
      // accidentally match the opening brace itself.
      var tail = updatedJs.slice(imagesOpenIdx + 1);
      var closeMatch = tail.match(/\n(\s*)\}(\s*;?)/);
      if (closeMatch) {
        // Absolute position of the `\n` that precedes the `}`
        var nlPos = imagesOpenIdx + 1 + tail.indexOf(closeMatch[0]);
        // Absolute position of the `}`
        var imagesClose = nlPos + closeMatch[0].indexOf("}");
        // Check whether the last non-whitespace character before the closing } is already a comma.
        var charBeforeClose = updatedJs.slice(0, nlPos).trimEnd().slice(-1);
        var needsLeadingComma = (charBeforeClose !== ",");
        var separator = needsLeadingComma ? ",\n" : "\n";
        updatedJs = updatedJs.slice(0, imagesClose) + separator + newImageEntries + "\n" + updatedJs.slice(imagesClose);
      }
    } else {
      // No IMAGES block exists at all — create one
      updatedJs += "\n\nconst IMAGES = {\n" + newImageEntries + "\n};";
    }
  }
...
export default function VesselUploaderV4() {
  ...
  var colorFileName = COLOR_FILE_MAP[color] || color.toLowerCase();
    ...
    <div style={{ fontFamily:"DM Mono,monospace", background:"#F5F2EE", minHeight:"100vh" }}>
      ...
      <div style={{ background:"#1A1A18", color:"#fff", padding:"0 24px", height:52, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <span style={{ fontFamily:"DM Mono,monospace", fontSize:9, color:"#555", background:"#2A2A28", border:"1px solid #3A3A38", borderRadius:3, padding:"2px 6px", letterSpacing:"0.08em" }}>V4</span>
        </div>
      </div>
        ...
        <div style={{ maxWidth:660, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
            {[
              ...
              ["STEP 3","Download Updated JS","Click Download — the artifact merges your new vessels into the color JS file directly. No Claude merge step needed."]
            ].map(function(s) {
              return (
                <div key={s[0]}>
      ...
      <div style={{ maxWidth:660, margin:"0 auto", padding:"24px 20px" }}>
        ...
        <div style={card}>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              ...
              <a href={"https://raw.githubusercontent.com/photokitchenfood/pk-vessels-catalog/refs/heads/main/data/" + colorFileName + ".js"} target="_blank" rel="noreferrer"
            ...
            <div style={{ marginTop:6, fontSize:10, color:"#C4C0B8", lineHeight:1.6 }}>
              {"Download " + colorFileName + ".js then drag it below. If it opens as text in your browser, right-click → Save Link As."}
            </div>
          </div>
        ...
        {queue.length > 0 && (
          <div style={card}>
            ...
            <button
              onClick={function() {
                mergeAndDownload(rawJs, queue, color);
                setDownloaded(true);
              }}
            ...
            {downloaded && (
              <div style={{ background:"#F0FBF0", border:"1px solid #4CAF5055", borderRadius:6, padding:"12px 14px", fontSize:10, color:"#2E7D32", lineHeight:1.7, fontFamily:"DM Mono,monospace" }}>
                <strong style={{ display:"block", marginBottom:4 }}>{"✓ " + colorFileName + ".js downloaded — ready to push."}</strong>
                New vessels are already merged in. Just drag <strong>{colorFileName + ".js"}</strong> into GitHub Desktop and push.
              </div>
            )}
          </div>
        )}
